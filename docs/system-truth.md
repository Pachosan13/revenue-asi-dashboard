...
## Changelog
- ENC24 GHL dispatch send gate tightened in `worker/run-enc24-ghl-dispatch.mjs`: even with listing-key dedupe, webhook send now still requires valid `phone_e164` (`+` prefix) and otherwise marks `missing_listing_or_phone` as failed (no blocked quarantine path).
- ENC24 GHL listing-identity dedupe fix in `worker/run-enc24-ghl-dispatch.mjs`: dedupe no longer keys by phone; it now checks recent sent deliveries by payload listing identity (`listing_url_hash` preferred, fallback `listing_url`/`url`) and payload includes `listing_url_hash` for stable matching.
...
- ENC24 GHL dispatch worker guardrails added in `worker/run-enc24-ghl-dispatch.mjs` immediately before webhook POST: env-driven phone dedupe (`ENC24_DEDUPE_ENABLED`, `ENC24_DEDUPE_HOURS`, `ENC24_DEDUPE_DRYRUN`) checks `enc24_ghl_deliveries.payload->>'phone_e164'` in a recent window, and env-driven dealer-cluster checks (`ENC24_DEALER_CLUSTER_ENABLED`, `ENC24_DEALER_CLUSTER_HOURS`, `ENC24_DEALER_CLUSTER_MIN`, `ENC24_DEALER_CLUSTER_DRYRUN`) count same-phone listings in `enc24_listings`; dry-run logs `event=dispatch_dedupe_dryrun|dealer_cluster_dryrun`, blocking mode logs `event=dispatch_dedupe_blocked|dealer_cluster_blocked` and quarantines delivery rows as `failed` with `next_attempt_at=now()+100 years` and `last_error=dispatch_blocked_*`.
- ENC24 reveal DOM alignment update in `worker/providers/phone-resolver/encuentra24_whatsapp_resolver.mjs`: Stage2 form fill now uses stable selectors (`#cnmessage_fromemail`, `#cnmessage_name`, `input.init-tel-input`), triggers field blur via `Tab`, waits up to 10s for `.d3-property-contact__phone.disabled` to clear and `input[name='cnmessage[captcha]']` token population, clicks the real `.show-phone` trigger (scroll + trial + real click), and keeps XHR-first extraction expanded to accept either `/cnad/suggestionsmodal/adid/<adId>` or `POST /panama-es/cnmessage/send/adid/<adId>` before DOM fallbacks while preserving seed guard and `+507` normalization.
- ENC24 overlay-safe click hardening in `worker/providers/phone-resolver/encuentra24_whatsapp_resolver.mjs`: before `Llamar`, resolver now runs `scrollIntoViewIfNeeded` on `text=Llamar`, re-runs `closeOverlays(page)`, executes trial click (`click({ trial:true })`) with phase logs (`phase=llamar_trial_ok|phase=llamar_trial_fail`) and `overlay_trial_failed` log on failure, then attempts real click with `phase=llamar_click_ok`; overlay-blocked failures now always trigger one in-run reload retry sequence (`reveal_retry_once reason=overlay_blocked`) while keeping XHR-first `waitForResponse(/cnad/suggestionsmodal/adid/<adId>)` intact and DOM fallback unchanged.
- ENC24 reveal XHR-first extraction update in `worker/providers/phone-resolver/encuentra24_whatsapp_resolver.mjs`: parse `adId` from the last numeric path segment in `listing_url`, start `page.waitForResponse` for `/cnad/suggestionsmodal/adid/<adId>` (HTTP 200) before clicking `Llamar`, parse JSON `{ success, content }`, normalize/validate phone from `content` (reject seed), log `phase=xhr_found adid=<adId>`, and keep `tel:` / `wa.me` DOM extraction as fallback when XHR is missing or times out.
- ENC24 Stage2 reveal safety guardrails: `worker/providers/phone-resolver/encuentra24_whatsapp_resolver.mjs` now uses shared `closeOverlays(page)` before fill, right before click `Llamar`, and right after click `Llamar`; adds one in-run retry (`reveal_retry_once reason=<reason>`) for `overlay_blocked|reveal_timeout_30s` controlled by `ENC24_REVEAL_RETRY_ON_TIMEOUT` (default `"1"`). Enqueue hard-stop for commercial outcomes is versioned in `supabase/migrations/20260303100000_enc24_no_retry_seller_name_commercial.sql` (`reason='seller_name_commercial'` excluded from enqueue/requeue candidates).
- ENC24 dealer exclusion guardrails are DB-enforced in `supabase/migrations/20260303113000_enc24_dealer_guardrails_dispatch.sql`: post-reveal trigger on `lead_hunter.enc24_listings` classifies `seller_name_commercial` (expanded seller-name keyword checks, including `MATIC`/`MOTORS`/`AUTOS`/`S.A.` variants) and `dealer_phone_cluster` (same `phone_e164` in `>=2` listings per account within 24h), and dispatch trigger on `lead_hunter.enc24_ghl_deliveries` blocks enqueue/sending for commercial/cluster rows by forcing them to non-due `failed` with `dispatch_blocked_*` errors.
- ENC24 autopilot loop observability/safety update (`worker/run-enc24-autos-autopilot.mjs`): added deterministic phase logs (`phase=tick_begin|settings_load_start|settings_load_done|collect_start|collect_done|reveal_queue_start|reveal_queue_done|reveal_run_start|reveal_run_done|ghl_dispatch_start|ghl_dispatch_done|tick_done`), 60s tick watchdog logging `tick_timeout_60s phase=<last_phase>`, 20s timeout on every Postgres query with forced client close/reconnect on timeout, and session-level `SET statement_timeout=20000` + `SET lock_timeout=5000` after connect.
- ENC24 autopilot tick loop (`worker/run-enc24-autos-autopilot.mjs`) now enforces bounded fail-open execution: per-tick timeout (`tick_timeout_60s`), collect timeout (`collect_timeout_25s` via `AbortController`), timeout-wrapped PG/child-process phases, phase boundary logs (`phase=*_start|*_done`), and unconditional `tick done` logging so loop execution continues after per-phase/per-tick failures.
- ENC24 worker Docker build now installs browser runtime with `npx playwright install chromium --with-deps` (explicit chromium target) after `npm install --production`.
- ENC24 worker Docker runtime sets `PLAYWRIGHT_CHROMIUM_USE_HEADLESS_SHELL=0` so Playwright headless launch uses Chromium (not `chromium_headless_shell`) in Fly.
- ENC24 resolver launch policy hardened: `worker/providers/phone-resolver/encuentra24_whatsapp_resolver.mjs` no longer uses `executablePath`, `launchPersistentContext`, or `connectOverCDP`; each run dynamically imports Playwright and uses only `await chromium.launch({ headless: true, args: ["--no-sandbox","--disable-dev-shm-usage"] })`, logging `reveal_mode=launch platform=<platform> arch=<arch>` to verify runtime target (Fly should report `linux` + `x64`).
- ENC24 reveal runtime defaults in autopilot/worker remain `ENC24_CDP="0"` with empty `ENC24_CDP_URL`; resolver execution path is now launch-only (`reveal_mode=launch`) and no longer attempts CDP.
- ENC24 stage semantics v2: `lead_hunter.enc24_listings.stage` is now textual, stage1 collector persists `ok/stage/reason` deterministically (`stage1_ok` or `stage1_filtered` + short reason), `ok=false` rows are backfilled/guarded to always include `stage` + `reason`, and `lead_hunter.enqueue_enc24_reveal_tasks` now queues only `ok=true AND stage='stage1_ok'` rows (failed tasks for eligible listings are requeued immediately; done tasks are not requeued). Collector/autopilot logs now include per-tick metrics: `collected_total`, `inserted`, `ok_true`, `ok_false_by_reason`, `enqueued_reveal`.
- ENC24 enqueue RPC now drops legacy overload `lead_hunter.enqueue_enc24_reveal_tasks(integer, integer)` to prevent ambiguous one-arg calls from stage1 collector/autopilot.
- ENC24 enqueue RPC eligibility now includes `stage in ('stage1_ok','reveal_failed')` (with `ok=true` and no `phone_e164`) so transient reveal failures can be requeued on later ticks.
- ENC24 enqueue RPC retries `stage='reveal_failed'` listings even when `ok=false` (still requires valid listing URL, missing `phone_e164`, and excludes commercial reasons), while stage1-first enqueue remains `ok=true AND stage='stage1_ok'`.
- ENC24 reveal Stage2 playbook restored in `worker/providers/phone-resolver/encuentra24_whatsapp_resolver.mjs`: resolver now uses strict contact-panel scoping (`text=Enviar mensaje al vendedor` + nearest `section|div|form`), fills only visible email/name/tel fields inside that panel, closes common overlays before interactions, clicks `Llamar` without force, extracts phone in deterministic priority (`tel:` DOM → WhatsApp DOM → XHR/fetch response), rejects seed phone values (`67777777` / configured seed), emits phase logs (`phase=panel_found|filled_form|clicked_llamar|tel_dom_found|wa_dom_found|xhr_found`), writes fail screenshots after fill/click, and returns short reason codes (`overlay_blocked|no_tel_dom|no_wa_dom|no_xhr_phone|seed_only`) on failure.
- Docker runtime for ENC24 worker (`Dockerfile`) now installs `ca-certificates` and refreshes trust store (`update-ca-certificates`) before `npm install` so Node TLS validation has system CA chain available in Fly (`node:20-slim`) for Supabase Postgres connections.
- ENC24 workers share `worker/lib/pg-config.mjs` for Postgres connection policy: for Supabase hosts (`*.supabase.co`), config is built from parsed `DATABASE_URL` fields (`host`, `port`, `database`, `user`, `password`) and uses `ssl: { rejectUnauthorized: false }` without passing `connectionString` (URL SSL query params are ignored). Localhost uses plain `connectionString` without SSL. Boot logs include `pg_connect: host=<host> ssl=<bool> rejectUnauthorized=<bool>` and `pg_ssl_object: <typeof ssl> keys=<Object.keys(ssl)>` (no secrets). `run-enc24-autos-autopilot.mjs` supports `RUN_ONCE=1` (connect DB, read `lead_hunter.enc24_autopilot_settings`, exit 0).
