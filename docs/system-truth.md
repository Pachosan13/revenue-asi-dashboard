...
## Changelog
- ENC24 workers now share `worker/lib/pg-config.mjs` for Postgres connection policy: auto-detect Supabase host (`.supabase.co`) or `SUPABASE_DB_SSL=1` to enable SSL with `rejectUnauthorized=false` by default (override via `PG_SSL` / `PG_SSL_REJECT_UNAUTHORIZED`), keep localhost without SSL, and emit boot log `pg_connect: host=<host> ssl=<bool> rejectUnauthorized=<bool>` (no secrets). `run-enc24-autos-autopilot.mjs` also supports `RUN_ONCE=1` (connect DB, read `lead_hunter.enc24_autopilot_settings`, exit 0).
