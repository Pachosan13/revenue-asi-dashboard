...
## Changelog
- Voice: Autos Panama voice agent now speaks as “Juan Carlos” (persona, not empresa), asks only greet→availability→urgency before done, and HOT handoff line passes the number to Juan Carlos (no dealer/compañía mention). See `services/voice-rtp-gateway/server.js`.
...
- Voice: Bot turn close is deterministic: requires `openai_done` plus Telnyx mark ack or a silence window after `response.output_audio.done` (no `response_done_timeout` auto-close). Logs `close_reason` (`telnyx_mark_ack`|`silence_after_audio`|`forced_hangup`). See `services/voice-rtp-gateway/server.js`.
