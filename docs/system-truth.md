...
- ENC24 workers share `worker/lib/pg-config.mjs` for Postgres connection policy: when host is Supabase (`.supabase.co`) or `SUPABASE_DB_SSL=1`, the client is created with a real SSL object `ssl: { rejectUnauthorized: false }`; localhost remains non-SSL. Boot logs include `pg_connect: host=<host> ssl=<bool> rejectUnauthorized=<bool>` and `pg_ssl_object: <typeof ssl> keys=<Object.keys(ssl)>` (no secrets). `run-enc24-autos-autopilot.mjs` supports `RUN_ONCE=1` (connect DB, read `lead_hunter.enc24_autopilot_settings`, exit 0).
