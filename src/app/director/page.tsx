...
import { Mail, PhoneCall, MessageSquare, Zap } from "lucide-react"
...
// 🧠 Lead Next Action + Genome v2

type DirectorNextActionRow = {
  lead_id: string
...
async function loadNextActions(
  client: ReturnType<typeof supabaseServer>,
): Promise<DirectorNextActionRow[]> {
  if (!client) return []

  // 1) Vista v5 (ya con lógica del Brain)
  const { data, error } = await client
  ...
  if (leadIds.length > 0) {
    const { data: genomeData, error: genomeError } = await client
      ...
      .select(
        ["id", "industry", "sub_industry", "enrichment_status", "ai_lead_score"]
          .join(", "),
      )
      .in("id", leadIds)
  ...
  // 3) Merge: effective_channel + genome
  return rows.map((row) => {
...
async function loadEnrichmentOverview(
  ...
  // 2) Estado agregado: v_lead_with_enrichment_v2
      ...
      <CardContent className="space-y-4">
        {alerts.length === 0 ? (
          ...
          alerts.map((alert) => {
            ...
            const importance = payload.importance ?? alert.score_delta ?? 1
...
function NextActionsSection({ rows }: { rows: DirectorNextActionRow[] }) {
  return (
    <Card className="border-white/10 bg-white/5">
      <CardHeader
...
export default async function DirectorPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Lead Brain – Next actions */}
      <NextActionsSection rows={nextActions} />

      {/* Engines + snapshot + tiles */}
