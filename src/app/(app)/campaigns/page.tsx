...
export default function CampaignsPage() {
  ...
  const loadCampaigns = useCallback(async () => {
    ...
    const [
      ...
      { data: kpis },
      { data: orgSettings },
      { data: clRecent, error: ldErr },
      { count: leadsEnrolled, error: leErr },
      { count: contactsAttempted, error: caErr },
      { count: appointmentsBooked, error: abErr },
    ] =
      await Promise.all([
        supabase
          .from("campaigns")
      ...
      supabase.from("campaign_funnel_overview").select("*"),
        ...
        supabase
          .from("appointments")
          .select("id, leads!inner(account_id)", { count: "exact", head: true })
          .eq("leads.account_id", accountId),
      ])
    ...
    if (leErr) {
      ...
      setRuntimeError(`Failed to load conversion KPI (leads_enrolled): ${leErr.message}`)
      setLoading(false)
      return
    }
    if (caErr) {
      setConversionKpis(null)
      setRuntimeError(`Failed to load conversion KPI (contacts_attempted): ${caErr.message}`)
      setLoading(false)
      return
    }
    if (abErr) {
      setConversionKpis(null)
      setRuntimeError(`Failed to load conversion KPI (appointments_booked): ${abErr.message}`)
      setLoading(false)
      return
    }
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader
          ...
          description="Campaigns contact, qualify and convert leads generated by Programs."
          action={
        ...
        <CardContent className="space-y-4">
          <Card>
            ...
            />
            <CardContent className="grid gap-3 md:grid-cols-4 text-sm text-white/70">
