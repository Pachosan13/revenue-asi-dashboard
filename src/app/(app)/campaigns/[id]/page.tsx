...
export default function CampaignDetailPage() {
  ...
  const [conversionKpis, setConversionKpis] = useState<{
  ...
  useEffect(() => {
    async function loadCampaign() {
      ...
      setAccountId(typeof (data as any).account_id === "string" ? (data as any).account_id : null)
      setLoading(false)
    }
  ...
  // --------------------------------
  // Conversion KPIs (minimal, real tables)
  // --------------------------------
  useEffect(() => {
    let alive = true
    async function loadConversionKpis() {
      if (!params?.id || params.id === "new" || !accountId) return
      const campaignId = params.id

      const [{ count: leadsEnrolled, error: leErr }, { count: contactsAttempted, error: caErr }] =
        await Promise.all([
          supabase
            .from("campaign_leads")
      ...
      if (leErr || caErr) {
        setConversionKpis(null)
        return
      }

      // appointments_booked: appointments has lead_id only; we count appointments for leads enrolled in this campaign.
      const { data: leadRows, error: clErr } = await supabase
        .from("campaign_leads")
        .select("lead_id")
        .eq("account_id", accountId)
        .eq("campaign_id", campaignId)
      ...
      const leadIds = (Array.isArray(leadRows) ? leadRows : [])
        .map((r: any) => (r?.lead_id ? String(r.lead_id) : null))
      ...
      const enrolled = leadsEnrolled ?? 0
      setConversionKpis({
        leads_enrolled: enrolled,
        contacts_attempted: contactsAttempted ?? 0,
        appointments_booked: appointmentsBooked,
        conversion_rate: enrolled > 0 ? appointmentsBooked / enrolled : null,
      })
    }
    void loadConversionKpis()
    return () => {
      alive = false
    }
  }, [params?.id, accountId, supabase])
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader
          ...
          description="Campaigns convert existing supply."
        />
        <CardContent className="space-y-2 text-sm text-white/70">
          <p className="text-sm text-white/60">
            Campaigns contact, qualify and convert leads generated by Programs.
          </p>
          <div className="grid gap-3 md:grid-cols-4">
            <div className="rounded-2xl border border-white/10 bg-black/30 p-4">
              <div className="text-xs uppercase tracking-[0.16em] text-white/40">leads_enrolled</div>
              <div className="mt-1 text-2xl font-semibold text-white">
                {conversionKpis?.leads_enrolled ?? "—"}
              </div>
            </div>
            <div className="rounded-2xl border border-white/10 bg-black/30 p-4">
              <div className="text-xs uppercase tracking-[0.16em] text-white/40">contacts_attempted</div>
              <div className="mt-1 text-2xl font-semibold text-white">
                {conversionKpis?.contacts_attempted ?? "—"}
              </div>
            </div>
            <div className="rounded-2xl border border-white/10 bg-black/30 p-4">
              <div className="text-xs uppercase tracking-[0.16em] text-white/40">appointments_booked</div>
              <div className="mt-1 text-2xl font-semibold text-white">
                {conversionKpis?.appointments_booked ?? "—"}
              </div>
            </div>
            <div className="rounded-2xl border border-white/10 bg-black/30 p-4">
              <div className="text-xs uppercase tracking-[0.16em] text-white/40">conversion_rate</div>
              <div className="mt-1 text-2xl font-semibold text-white">
                {typeof conversionKpis?.conversion_rate === "number"
                  ? `${(conversionKpis.conversion_rate * 100).toFixed(1)}%`
                  : "—"}
              </div>
            </div>
          </div>
